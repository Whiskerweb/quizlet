-- ============================================
-- Migration 03: Class Modules System
-- Description: Allow teachers to share modules (folders) with their classes
-- Date: 2025-12-08
-- Prerequisites: 
--   - add_folders.sql must be executed first
--   - 02_add_classes_system.sql must be executed first
-- ============================================
...
BEGIN
  ...
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'classes') THEN
    RAISE EXCEPTION 'Table classes does not exist. Please run 02_add_classes_system.sql first.';
  END IF;
END $$;
...
DROP POLICY IF EXISTS "Teachers can view modules in their classes" ON public.class_modules;
...
DROP POLICY IF EXISTS "Teachers can add modules to their classes" ON public.class_modules;
...
DROP POLICY IF EXISTS "Teachers can remove modules from their classes" ON public.class_modules;
...
DROP POLICY IF EXISTS "Students can view modules in their classes" ON public.class_modules;
